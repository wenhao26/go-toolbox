...
// q 队列
// return 接收消息的通道 ， 错误对象
func (mq *MyMQ) Sub(q *Queue) (<-chan *Message, error) {
	subMu.Lock()
	defer subMu.Unlock()
...
// 将 mq 通道绑到队列通道中
func (mq *MyMQ) bindMQChan(q *Queue) error {
	bindMu.Lock()
	defer bindMu.Unlock()
	// 定义队列
	if !q.IsDeclare {
...
func (mq *MyMQ) Push(q *Queue, msg *Message, exchanges ...*Exchange) error {
...
// msg 消息,
// exchanges 交换机，可以用多个交换机多次发送，默认使用初始化时指定的交换机
func (mq *MyMQ) Pub(routingKey string, msg *Message, exchanges ...*Exchange) error {
	if mq.Client == nil || mq.Channel == nil {
		return errors.New("mq 未初始化")
	}
...
func (mq *MyMQ) QueueDeclare(q *Queue) error {
...
func (mq *MyMQ) ExchangeDeclare(e *Exchange) error {
	if e.IsDeclare {
		return nil
	}
...
func (mq *MyMQ) QueueBind(q *Queue, e *Exchange) error {
	if !e.IsDeclare {
